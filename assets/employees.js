...
const viewEmployees = async (connection, employees) => {
  connection.query(mainQuery, (err, rows) => {
    if (err) throw err;
  ...
  connection.query(
    ...
    async (err, rows) => {
      ...
      if (answers.chooseDepartment === "All") {
        connection.query(`${mainQuery} ORDER BY Department`, (err, rows) => {
          if (err) throw err;
          console.log("All Employees");
          console.table(rows);
          employees();
...
const viewEmployeesByManager = async (connection, employees) => {
  ...
  connection.query(
    "Select id, CONCAT(first_name, ' ', last_name) as manager from employees where manager_id is NULL",
    async (err, rows) => {
      if (err) throw err;
...
const editEmployeeRole = async (connection, employees) => {
  ...
  connection.query(
    ...
    [{ role_id: answers.role }, { id: answers.alter }],
    (err, rows) => {
      if (err) {
...
const editEmployeeManager = async (connection, employees) => {
  ...
  connection.query(
    `UPDATE employees set ? WHERE ?`,
    [{ manager_id: answers.manager }, { id: answers.alter }],
    (err, rows) => {
      if (err) {
